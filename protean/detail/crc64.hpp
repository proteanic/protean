#ifndef PROTEAN_DETAIL_CRC64_HPP
#define PROTEAN_DETAIL_CRC64_HPP

namespace protean { namespace detail {

    const boost::uint64_t crc_table[256] =
    {
    ...
    inline boost::uint64_t update_crc64(boost::uint64_t crc, const unsigned char* buffer, size_t length)
    {
        for (size_t i = 0; i < length; ++i)
        {
            crc = crc_table[(unsigned char)(crc >> 56) ^ *buffer++] ^ (crc << 8);
        }
        return crc;
    }
